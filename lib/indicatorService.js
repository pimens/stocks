...
class IndicatorService {
  ...
  calculateIndicatorsForRegression(prices) {
    ...
    return {
      ...
      rsi, rsi7, rsi21, macd, bb, stoch, adx, atr, obv,
      williamsR, cci, mfi, roc, momentum, pricePosition, volumeRatio, psar
    };
  }

  // Generate regression dataset with indicators from signal day (H)
  // and confirmation data from the next trading day (H+1)
  ...
  generateRegressionDataset(prices, startDate, endDate, options = {}) {
    const {
      ...
      horizonDays = 1,
      tpPercent = 5,          // Take profit (%) dari entry (close H)
      slPercent = 5           // Stop loss (%) dari entry (close H)
    } = options;
    const horizon = Math.max(1, parseInt(horizonDays, 10) || 1);
    const tp = Math.max(0, parseFloat(tpPercent) || 5);
    const sl = Math.max(0, parseFloat(slPercent) || 5);
    ...
    for (let i = 2; i < prices.length - horizon; i++) { // Need H+n confirmation data
      ...
      const signalIdx = i;
      const prevIdx = signalIdx;
      const prevPrevIdx = signalIdx - 1;
      const signalClose = prices[signalIdx].close;
      const signalOpen = prices[signalIdx].open;
      const nextDayIdx = signalIdx + 1;
      const outcomeIdx = signalIdx + horizon;
      const nextDayOpen = prices[nextDayIdx].open;
      const outcomeClose = prices[outcomeIdx].close;
      const priceChange = outcomeClose - signalClose;
      const priceChangePercent = signalClose > 0 ? ((outcomeClose - signalClose) / signalClose) * 100 : 0;

      // === TP/SL SIMULATION ===
      // Entry = close hari sinyal (H). TP/SL dicek harian dari H+1 s/d H+n (hari perdagangan).
      // Win = TP kena sebelum SL. Jika TP & SL tersentuh di hari yang sama -> dihitung SL duluan (konservatif).
      // Jika tidak tersentuh sampai H+n -> time exit di close H+n (bukan Win). Gap up tidak mengubah harga exit TP/SL.
      const entryPrice = signalClose;
      const tpPrice = entryPrice * (1 + tp / 100);
      const slPrice = entryPrice * (1 - sl / 100);
      let tpslStatus = 'TIME_EXIT';
      let tpslExitIdx = outcomeIdx;
      let tpslExitPrice = outcomeClose;
      let tpslDaysHeld = horizon;
      for (let d = 1; d <= horizon; d++) {
        const day = prices[signalIdx + d];
        if (!day) break;
        const hitTP = day.high >= tpPrice;
        const hitSL = day.low <= slPrice;
        if (hitTP || hitSL) {
          tpslStatus = hitSL ? 'SL' : 'TP';
          tpslExitIdx = signalIdx + d;
          tpslExitPrice = hitSL ? slPrice : tpPrice;
          tpslDaysHeld = d;
          break;
        }
      }
      const tpslPercent = tpslStatus === 'TP' ? tp : tpslStatus === 'SL' ? -sl : priceChangePercent;
      ...
      // === ML-FRIENDLY FEATURES FOR SIGNAL DAY (H) ===
      const prevClose = signalClose;
      const currentClose = signalClose;
      const currentOpen = nextDayOpen;
      const prevHigh = prices[signalIdx].high;
      const prevLow = prices[signalIdx].low;
      const prevOpen = signalOpen;
      ...
      const deltaRSI7 = (indicators.rsi7[prevIdx] !== null && indicators.rsi7[prevPrevIdx] !== null)
        ? indicators.rsi7[prevIdx] - indicators.rsi7[prevPrevIdx] : null;
      
      const deltaRSI21 = (indicators.rsi21[prevIdx] !== null && indicators.rsi21[prevPrevIdx] !== null)
        ? indicators.rsi21[prevIdx] - indicators.rsi21[prevPrevIdx] : null;
      ...
      const row = {
        ...
        futureDate: prices[outcomeIdx].date,
        futureClose: parseFloat(outcomeClose.toFixed(2)),
        gapOpenDate: prices[nextDayIdx].date,
        gapOpen: parseFloat(nextDayOpen?.toFixed(2) || 0),
        entryPrice: parseFloat(entryPrice.toFixed(2)),
        tpPrice: parseFloat(tpPrice.toFixed(2)),
        slPrice: parseFloat(slPrice.toFixed(2)),
        tpslStatus,
        tpslExitDate: prices[tpslExitIdx]?.date || prices[outcomeIdx].date,
        tpslExitPrice: parseFloat(tpslExitPrice.toFixed(2)),
        tpslPercent: parseFloat(tpslPercent.toFixed(4)),
        tpslDaysHeld,
        prevOpen: parseFloat(prevOpen?.toFixed(2) || 0),
        prevHigh: parseFloat(prevHigh?.toFixed(2) || 0),
        prevLow: parseFloat(prevLow?.toFixed(2) || 0),
        prevVolume: prices[signalIdx].volume || 0,
        ...
        deltaRSI7: deltaRSI7 !== null ? parseFloat(deltaRSI7.toFixed(2)) : null,
        deltaRSI21: deltaRSI21 !== null ? parseFloat(deltaRSI21.toFixed(2)) : null,
        ...
        // RSI 7 (H-1)
        rsi7: indicators.rsi7[prevIdx] ? parseFloat(indicators.rsi7[prevIdx].toFixed(2)) : null,
        rsi7Oversold: indicators.rsi7[prevIdx] && indicators.rsi7[prevIdx] < 30 ? 1 : 0,
        rsi7Overbought: indicators.rsi7[prevIdx] && indicators.rsi7[prevIdx] > 70 ? 1 : 0,
        rsi7Neutral: indicators.rsi7[prevIdx] && indicators.rsi7[prevIdx] >= 30 && indicators.rsi7[prevIdx] <= 70 ? 1 : 0,
        
        // RSI 21 (H-1)
        rsi21: indicators.rsi21[prevIdx] ? parseFloat(indicators.rsi21[prevIdx].toFixed(2)) : null,
        rsi21Oversold: indicators.rsi21[prevIdx] && indicators.rsi21[prevIdx] < 30 ? 1 : 0,
        rsi21Overbought: indicators.rsi21[prevIdx] && indicators.rsi21[prevIdx] > 70 ? 1 : 0,
        rsi21Neutral: indicators.rsi21[prevIdx] && indicators.rsi21[prevIdx] >= 30 && indicators.rsi21[prevIdx] <= 70 ? 1 : 0,
        ...
        // Candlestick patterns (signal day H)
        bodySize: parseFloat(Math.abs(prices[signalIdx].close - prices[signalIdx].open).toFixed(2)),
        upperWick: parseFloat((prices[signalIdx].high - Math.max(prices[signalIdx].open, prices[signalIdx].close)).toFixed(2)),
        lowerWick: parseFloat((Math.min(prices[signalIdx].open, prices[signalIdx].close) - prices[signalIdx].low).toFixed(2)),
        isBullishCandle: prices[signalIdx].close > prices[signalIdx].open ? 1 : 0,
        isDoji: Math.abs(prices[signalIdx].close - prices[signalIdx].open) < (prices[signalIdx].high - prices[signalIdx].low) * 0.1 ? 1 : 0,
        
        // Price gaps on signal day H
        gapUp: prices[signalIdx].open > prices[signalIdx - 1]?.close ? 1 : 0,
        gapDown: prices[signalIdx].open < prices[signalIdx - 1]?.close ? 1 : 0,
        
        // Daily returns on signal day H
        return1d: signalIdx >= 1 ? parseFloat(((prices[signalIdx].close - prices[signalIdx - 1].close) / prices[signalIdx - 1].close * 100).toFixed(4)) : null,
        return3d: signalIdx >= 3 ? parseFloat(((prices[signalIdx].close - prices[signalIdx - 3].close) / prices[signalIdx - 3].close * 100).toFixed(4)) : null,
        return5d: signalIdx >= 5 ? parseFloat(((prices[signalIdx].close - prices[signalIdx - 5].close) / prices[signalIdx - 5].close * 100).toFixed(4)) : null,
        ...
        // RSI 7 Momentum
        rsi7Rising: (prevIdx >= 1 && indicators.rsi7[prevIdx] && indicators.rsi7[prevIdx - 1] &&
          indicators.rsi7[prevIdx] > indicators.rsi7[prevIdx - 1]) ? 1 : 0,
        rsi7ExitOversold: (prevIdx >= 1 && indicators.rsi7[prevIdx] && indicators.rsi7[prevIdx - 1] &&
          indicators.rsi7[prevIdx - 1] < 30 && indicators.rsi7[prevIdx] >= 30) ? 1 : 0,
        rsi7BullishZone: (indicators.rsi7[prevIdx] && indicators.rsi7[prevIdx] >= 30 && indicators.rsi7[prevIdx] <= 50) ? 1 : 0,
        
        // RSI 21 Momentum
        rsi21Rising: (prevIdx >= 1 && indicators.rsi21[prevIdx] && indicators.rsi21[prevIdx - 1] &&
          indicators.rsi21[prevIdx] > indicators.rsi21[prevIdx - 1]) ? 1 : 0,
        rsi21ExitOversold: (prevIdx >= 1 && indicators.rsi21[prevIdx] && indicators.rsi21[prevIdx - 1] &&
          indicators.rsi21[prevIdx - 1] < 30 && indicators.rsi21[prevIdx] >= 30) ? 1 : 0,
        rsi21BullishZone: (indicators.rsi21[prevIdx] && indicators.rsi21[prevIdx] >= 30 && indicators.rsi21[prevIdx] <= 50) ? 1 : 0,
  ...
  // Get indicator data for a specific signal date (H)
  // targetDate: the date whose indicators should be evaluated against the rules
  // timeframe: aggregate daily data to N-day candles (1=daily, 3=3-day, 5=weekly, etc.)
  // Returns indicators from H plus futureData from the next trading day (H+1)
  getIndicatorsForDate(prices, targetDate, timeframe = 1, horizonDays = 1) {
    ...
    // Index mapping:
    // i = target screening date (the date user selected)
    // i-1 = previous day (used for delta/change calculations)
    // i-2 = two days back (used for cross signal comparisons)
    const i = targetIdx;  // The selected screening date
    const prevIdx = i - 1;  // Previous day for comparisons
    const prevPrevIdx = i - 2;  // Two days back
    
    // CURRENT DAY OHLCV (the screening date - what user selected)
    const currentClose = resampledPrices[i].close;
    const currentHigh = resampledPrices[i].high;
    const currentLow = resampledPrices[i].low;
    const currentOpen = resampledPrices[i].open;
    const currentRange = currentHigh - currentLow;
    
    // PREVIOUS DAY OHLCV (for delta/gap calculations)
    const prevClose = resampledPrices[prevIdx]?.close || currentClose;
    const prevHigh = resampledPrices[prevIdx]?.high || currentHigh;
    const prevLow = resampledPrices[prevIdx]?.low || currentLow;
    const prevOpen = resampledPrices[prevIdx]?.open || currentOpen;
    const prevRange = prevHigh - prevLow;
    
    // Close Position (of the selected date)
    const closePosition = currentRange > 0 ? (currentClose - currentLow) / currentRange : 0.5;
    
    // Body/Range Ratio (of the selected date)
    const bodySize = Math.abs(currentClose - currentOpen);
    const bodyRangeRatio = currentRange > 0 ? bodySize / currentRange : 0;
    
    // Delta indicators (change from previous day to selected date)
    const deltaRSI = (indicators.rsi[i] !== null && indicators.rsi[prevIdx] !== null)
      ? indicators.rsi[i] - indicators.rsi[prevIdx] : null;
    
    const deltaRSI7 = (indicators.rsi7[i] !== null && indicators.rsi7[prevIdx] !== null)
      ? indicators.rsi7[i] - indicators.rsi7[prevIdx] : null;
    
    const deltaRSI21 = (indicators.rsi21[i] !== null && indicators.rsi21[prevIdx] !== null)
      ? indicators.rsi21[i] - indicators.rsi21[prevIdx] : null;
    ...
    const futureData = nextDayIdx < resampledPrices.length ? {
      date: resampledPrices[nextDayIdx].date,
      open: resampledPrices[nextDayIdx].open,
      high: resampledPrices[nextDayIdx].high,
      low: resampledPrices[nextDayIdx].low,
      close: resampledPrices[nextDayIdx].close,
      volume: resampledPrices[nextDayIdx].volume,
      priceChange: resampledPrices[nextDayIdx].close - currentClose,
      priceChangePercent: ((resampledPrices[nextDayIdx].close - currentClose) / currentClose) * 100,
      horizonDays: 1
    } : null;
    ...
    const row = {
      ...
      indicatorDate: resampledPrices[i].date,  // Indicators are FROM the selected date, not previous day
      ...
      // Price data for the selected date (screening date)
      prevClose: parseFloat(currentClose.toFixed(2)),  // renamed from prevClose - this is actually the screening date close
      prevOpen: parseFloat(currentOpen?.toFixed(2) || 0),
      prevHigh: parseFloat(currentHigh?.toFixed(2) || 0),
      prevLow: parseFloat(currentLow?.toFixed(2) || 0),
      prevVolume: resampledPrices[i].volume || 0,
      
      // ML Features (from the selected date)
      closePosition: parseFloat(closePosition.toFixed(4)),
      bodyRangeRatio: parseFloat(bodyRangeRatio.toFixed(4)),
      upperWickRatio: currentRange > 0 ? parseFloat(((currentHigh - Math.max(currentOpen, currentClose)) / currentRange).toFixed(4)) : 0,
      lowerWickRatio: currentRange > 0 ? parseFloat(((Math.min(currentOpen, currentClose) - currentLow) / currentRange).toFixed(4)) : 0,
      ...
      // SMA indicators (from selected date)
      sma5: indicators.sma5[i] ? parseFloat(indicators.sma5[i].toFixed(2)) : null,
      sma10: indicators.sma10[i] ? parseFloat(indicators.sma10[i].toFixed(2)) : null,
      sma20: indicators.sma20[i] ? parseFloat(indicators.sma20[i].toFixed(2)) : null,
      sma50: indicators.sma50[i] ? parseFloat(indicators.sma50[i].toFixed(2)) : null,
      sma100: indicators.sma100[i] ? parseFloat(indicators.sma100[i].toFixed(2)) : null,
      sma200: indicators.sma200[i] ? parseFloat(indicators.sma200[i].toFixed(2)) : null,
      
      // EMA indicators (from selected date)
      ema5: indicators.ema5[i] ? parseFloat(indicators.ema5[i].toFixed(2)) : null,
      ema10: indicators.ema10[i] ? parseFloat(indicators.ema10[i].toFixed(2)) : null,
      ema12: indicators.ema12[i] ? parseFloat(indicators.ema12[i].toFixed(2)) : null,
      ema26: indicators.ema26[i] ? parseFloat(indicators.ema26[i].toFixed(2)) : null,
      ema21: indicators.ema21[i] ? parseFloat(indicators.ema21[i].toFixed(2)) : null,
      ema21High: indicators.ema21High[i] ? parseFloat(indicators.ema21High[i].toFixed(2)) : null,
      ema21Low: indicators.ema21Low[i] ? parseFloat(indicators.ema21Low[i].toFixed(2)) : null,
      
      // Price vs MA signals (selected date price vs selected date MA)
      priceAboveSMA5: currentClose > (indicators.sma5[i] || 0) ? 1 : 0,
      priceAboveSMA10: currentClose > (indicators.sma10[i] || 0) ? 1 : 0,
      priceAboveSMA20: currentClose > (indicators.sma20[i] || 0) ? 1 : 0,
      priceAboveSMA50: currentClose > (indicators.sma50[i] || 0) ? 1 : 0,
      priceAboveEMA12: currentClose > (indicators.ema12[i] || 0) ? 1 : 0,
      priceAboveEMA26: currentClose > (indicators.ema26[i] || 0) ? 1 : 0,
      priceAboveEMA21: currentClose > (indicators.ema21[i] || 0) ? 1 : 0,
      priceAboveEMA21High: currentClose > (indicators.ema21High[i] || 0) ? 1 : 0,
      priceBelowEMA21Low: currentClose < (indicators.ema21Low[i] || Infinity) ? 1 : 0,
      
      // Distance from EMA 21
      distFromEMA21: indicators.ema21[i] ? parseFloat(((currentClose - indicators.ema21[i]) / indicators.ema21[i] * 100).toFixed(4)) : null,
      distFromEMA21High: indicators.ema21High[i] ? parseFloat(((currentClose - indicators.ema21High[i]) / indicators.ema21High[i] * 100).toFixed(4)) : null,
      distFromEMA21Low: indicators.ema21Low[i] ? parseFloat(((currentClose - indicators.ema21Low[i]) / indicators.ema21Low[i] * 100).toFixed(4)) : null,
      
      // EMA 21 Cross signals (price crossed above/below on selected date)
      priceCrossAboveEMA21: prevIdx >= 0 && resampledPrices[prevIdx]?.close && indicators.ema21[prevIdx] && indicators.ema21[i]
        ? (resampledPrices[prevIdx].close <= indicators.ema21[prevIdx] && currentClose > indicators.ema21[i] ? 1 : 0)
        : 0,
      priceCrossBelowEMA21: prevIdx >= 0 && resampledPrices[prevIdx]?.close && indicators.ema21[prevIdx] && indicators.ema21[i]
        ? (resampledPrices[prevIdx].close >= indicators.ema21[prevIdx] && currentClose < indicators.ema21[i] ? 1 : 0)
        : 0,
      priceCrossUpEMA21High: prevIdx >= 0 && resampledPrices[prevIdx]?.close && indicators.ema21High[prevIdx] && indicators.ema21High[i]
        ? (resampledPrices[prevIdx].close <= indicators.ema21High[prevIdx] && currentClose > indicators.ema21High[i] ? 1 : 0)
        : 0,
      
      // Distance from MA (on selected date)
      distFromSMA5: indicators.sma5[i] ? parseFloat(((currentClose - indicators.sma5[i]) / indicators.sma5[i] * 100).toFixed(4)) : null,
      distFromSMA20: indicators.sma20[i] ? parseFloat(((currentClose - indicators.sma20[i]) / indicators.sma20[i] * 100).toFixed(4)) : null,
      distFromSMA50: indicators.sma50[i] ? parseFloat(((currentClose - indicators.sma50[i]) / indicators.sma50[i] * 100).toFixed(4)) : null,
      
      // SMA crossover signals (on selected date)
      sma5AboveSMA10: (indicators.sma5[i] || 0) > (indicators.sma10[i] || 0) ? 1 : 0,
      sma10AboveSMA20: (indicators.sma10[i] || 0) > (indicators.sma20[i] || 0) ? 1 : 0,
      sma20AboveSMA50: (indicators.sma20[i] || 0) > (indicators.sma50[i] || 0) ? 1 : 0,
      
      // Golden Cross MA (fast MA crossing above slow MA on selected date)
      ma5CrossAboveMa10: (prevIdx >= 0 && 
        (indicators.sma5[prevIdx] || 0) <= (indicators.sma10[prevIdx] || 0) &&
        (indicators.sma5[i] || 0) > (indicators.sma10[i] || 0)) ? 1 : 0,
      ma10CrossAboveMa20: (prevIdx >= 0 && 
        (indicators.sma10[prevIdx] || 0) <= (indicators.sma20[prevIdx] || 0) &&
        (indicators.sma10[i] || 0) > (indicators.sma20[i] || 0)) ? 1 : 0,
      ma20CrossAboveMa50: (prevIdx >= 0 && 
        (indicators.sma20[prevIdx] || 0) <= (indicators.sma50[prevIdx] || 0) &&
        (indicators.sma20[i] || 0) > (indicators.sma50[i] || 0)) ? 1 : 0,
      ma50CrossAboveMa100: (prevIdx >= 0 && 
        (indicators.sma50[prevIdx] || 0) <= (indicators.sma100[prevIdx] || 0) &&
        (indicators.sma50[i] || 0) > (indicators.sma100[i] || 0)) ? 1 : 0,
      ma100CrossAboveMa200: (prevIdx >= 0 && 
        (indicators.sma100[prevIdx] || 0) <= (indicators.sma200[prevIdx] || 0) &&
        (indicators.sma100[i] || 0) > (indicators.sma200[i] || 0)) ? 1 : 0,
      
      // Death Cross MA (fast MA crossing below slow MA on selected date)
      ma5CrossBelowMa10: (prevIdx >= 0 && 
        (indicators.sma5[prevIdx] || 0) >= (indicators.sma10[prevIdx] || 0) &&
        (indicators.sma5[i] || 0) < (indicators.sma10[i] || 0)) ? 1 : 0,
      ma10CrossBelowMa20: (prevIdx >= 0 && 
        (indicators.sma10[prevIdx] || 0) >= (indicators.sma20[prevIdx] || 0) &&
        (indicators.sma10[i] || 0) < (indicators.sma20[i] || 0)) ? 1 : 0,
      ma20CrossBelowMa50: (prevIdx >= 0 && 
        (indicators.sma20[prevIdx] || 0) >= (indicators.sma50[prevIdx] || 0) &&
        (indicators.sma20[i] || 0) < (indicators.sma50[i] || 0)) ? 1 : 0,
      ma50CrossBelowMa100: (prevIdx >= 0 && 
        (indicators.sma50[prevIdx] || 0) >= (indicators.sma100[prevIdx] || 0) &&
        (indicators.sma50[i] || 0) < (indicators.sma100[i] || 0)) ? 1 : 0,
      ma100CrossBelowMa200: (prevIdx >= 0 && 
        (indicators.sma100[prevIdx] || 0) >= (indicators.sma200[prevIdx] || 0) &&
        (indicators.sma100[i] || 0) < (indicators.sma200[i] || 0)) ? 1 : 0,
      
      // RSI (from selected date)
      ...
      // RSI 7 (from selected date)
      rsi7: indicators.rsi7[i] ? parseFloat(indicators.rsi7[i].toFixed(2)) : null,
      rsi7Oversold: indicators.rsi7[i] && indicators.rsi7[i] < 30 ? 1 : 0,
      rsi7Overbought: indicators.rsi7[i] && indicators.rsi7[i] > 70 ? 1 : 0,
      rsi7Neutral: indicators.rsi7[i] && indicators.rsi7[i] >= 30 && indicators.rsi7[i] <= 70 ? 1 : 0,
      
      // RSI 21 (from selected date)
      rsi21: indicators.rsi21[i] ? parseFloat(indicators.rsi21[i].toFixed(2)) : null,
      rsi21Oversold: indicators.rsi21[i] && indicators.rsi21[i] < 30 ? 1 : 0,
      rsi21Overbought: indicators.rsi21[i] && indicators.rsi21[i] > 70 ? 1 : 0,
      rsi21Neutral: indicators.rsi21[i] && indicators.rsi21[i] >= 30 && indicators.rsi21[i] <= 70 ? 1 : 0,
      
      // MACD (from selected date)
      ...
      macdBullish: indicators.macd[i] && indicators.macd[i].MACD > indicators.macd[i].signal ? 1 : 0,
      macdPositive: indicators.macd[i] && indicators.macd[i].MACD > 0 ? 1 : 0,
      macdGoldenCross: (prevIdx >= 0 && indicators.macd[i] && indicators.macd[prevIdx] &&
        indicators.macd[prevIdx].MACD <= indicators.macd[prevIdx].signal &&
        indicators.macd[i].MACD > indicators.macd[i].signal) ? 1 : 0,
      macdDeathCross: (prevIdx >= 0 && indicators.macd[i] && indicators.macd[prevIdx] &&
        indicators.macd[prevIdx].MACD >= indicators.macd[prevIdx].signal &&
        indicators.macd[i].MACD < indicators.macd[i].signal) ? 1 : 0,
      // MACD Near Golden Cross Detection
      // Histogram negatif tapi naik (converging ke signal line)
      macdNearGoldenCross: (prevIdx >= 0 && indicators.macd[i] && indicators.macd[prevIdx] &&
      ...
      // MACD Momentum: berapa hari histogram naik berturut-turut
      macdHistogramRising: (prevIdx >= 0 && i >= 1 && indicators.macd[i] && indicators.macd[prevIdx] && indicators.macd[prevIdx - 1] &&
      ...
      macdDistanceToSignal: indicators.macd[i] && indicators.macd[i].signal !== 0 ? 
        parseFloat(((indicators.macd[i].MACD - indicators.macd[i].signal) / Math.abs(indicators.macd[i].signal) * 100).toFixed(2)) : null,
      
      // Bollinger Bands (from selected date)
      bbUpper: indicators.bb[i]?.upper ? parseFloat(indicators.bb[i].upper.toFixed(2)) : null,
      bbMiddle: indicators.bb[i]?.middle ? parseFloat(indicators.bb[i].middle.toFixed(2)) : null,
      bbLower: indicators.bb[i]?.lower ? parseFloat(indicators.bb[i].lower.toFixed(2)) : null,
      bbWidth: indicators.bb[i] ? parseFloat(((indicators.bb[i].upper - indicators.bb[i].lower) / indicators.bb[i].middle * 100).toFixed(4)) : null,
      priceBelowLowerBB: indicators.bb[i] && currentClose < indicators.bb[i].lower ? 1 : 0,
      priceAboveUpperBB: indicators.bb[i] && currentClose > indicators.bb[i].upper ? 1 : 0,
      
      // Stochastic (from selected date)
      stochK: indicators.stoch[i]?.k ? parseFloat(indicators.stoch[i].k.toFixed(2)) : null,
      stochD: indicators.stoch[i]?.d ? parseFloat(indicators.stoch[i].d.toFixed(2)) : null,
      ...
      stochBullishCross: indicators.stoch[i] && indicators.stoch[i].k > indicators.stoch[i].d ? 1 : 0,
      
      // ADX (from selected date)
      adx: indicators.adx[i]?.adx ? parseFloat(indicators.adx[i].adx.toFixed(2)) : null,
      pdi: indicators.adx[i]?.pdi ? parseFloat(indicators.adx[i].pdi.toFixed(2)) : null,
      mdi: indicators.adx[i]?.mdi ? parseFloat(indicators.adx[i].mdi.toFixed(2)) : null,
      ...
      // ATR (from selected date)
      atr: indicators.atr[i] ? parseFloat(indicators.atr[i].toFixed(2)) : null,
      atrPercent: indicators.atr[i] ? parseFloat((indicators.atr[i] / currentClose * 100).toFixed(4)) : null,
      
      // OBV (from selected date)
      obv: indicators.obv[i] || 0,
      obvChange: (i >= 1 && indicators.obv[i] !== null && indicators.obv[prevIdx] !== null) 
        ? indicators.obv[i] - indicators.obv[prevIdx] 
        : null,
      obvTrend: (i >= 1 && indicators.obv[i] !== null && indicators.obv[prevIdx] !== null)
        ? (indicators.obv[i] > indicators.obv[prevIdx] ? 1 : (indicators.obv[i] < indicators.obv[prevIdx] ? -1 : 0))
        : null,
      
      // Williams %R (from selected date)
      williamsR: indicators.williamsR[i] ? parseFloat(indicators.williamsR[i].toFixed(2)) : null,
      ...
      // CCI (from selected date)
      cci: indicators.cci[i] ? parseFloat(indicators.cci[i].toFixed(2)) : null,
      cciOversold: indicators.cci[i] && indicators.cci[i] < -100 ? 1 : 0,
      cciOverbought: indicators.cci[i] && indicators.cci[i] > 100 ? 1 : 0,
      
      // MFI (from selected date)
      mfi: indicators.mfi[i] ? parseFloat(indicators.mfi[i].toFixed(2)) : null,
      mfiOversold: indicators.mfi[i] && indicators.mfi[i] < 20 ? 1 : 0,
      mfiOverbought: indicators.mfi[i] && indicators.mfi[i] > 80 ? 1 : 0,
      
      // ROC (from selected date)
      roc: indicators.roc[i] ? parseFloat(indicators.roc[i].toFixed(4)) : null,
      rocPositive: indicators.roc[i] && indicators.roc[i] > 0 ? 1 : 0,
      
      // Momentum (from selected date)
      momentum: indicators.momentum[i] ? parseFloat(indicators.momentum[i].toFixed(2)) : null,
      momentumPositive: indicators.momentum[i] && indicators.momentum[i] > 0 ? 1 : 0,
      
      // Price Position (from selected date)
      pricePosition: indicators.pricePosition[i] ? parseFloat(indicators.pricePosition[i].toFixed(2)) : null,
      
      // Volume Ratio (from selected date)
      volumeRatio: indicators.volumeRatio[i] ? parseFloat(indicators.volumeRatio[i].toFixed(4)) : null,
      highVolume: indicators.volumeRatio[i] && indicators.volumeRatio[i] > 1.5 ? 1 : 0,
      
      // Candlestick patterns (from selected date)
      bodySize: parseFloat(Math.abs(currentClose - currentOpen).toFixed(2)),
      upperWick: parseFloat((currentHigh - Math.max(currentOpen, currentClose)).toFixed(2)),
      lowerWick: parseFloat((Math.min(currentOpen, currentClose) - currentLow).toFixed(2)),
      isBullishCandle: currentClose > currentOpen ? 1 : 0,
      isDoji: Math.abs(currentClose - currentOpen) < (currentHigh - currentLow) * 0.1 ? 1 : 0,
      
      // Price gaps (compare current open with previous close)
      gapUp: currentOpen > (resampledPrices[prevIdx]?.close || currentClose) ? 1 : 0,
      gapDown: currentOpen < (resampledPrices[prevIdx]?.close || currentClose) ? 1 : 0,
      
      // Returns (based on selected date)
      return1d: i >= 1 ? parseFloat(((currentClose - resampledPrices[prevIdx].close) / resampledPrices[prevIdx].close * 100).toFixed(4)) : null,
      return3d: i >= 3 ? parseFloat(((currentClose - resampledPrices[i - 3].close) / resampledPrices[i - 3].close * 100).toFixed(4)) : null,
      return5d: i >= 5 ? parseFloat(((currentClose - resampledPrices[i - 5].close) / resampledPrices[i - 5].close * 100).toFixed(4)) : null,
      
      // ============ ADVANCED BULLISH DETECTION INDICATORS ============
      
      // RSI Momentum: RSI naik pada selected date vs previous day
      rsiRising: (prevIdx >= 0 && indicators.rsi[i] && indicators.rsi[prevIdx] &&
        indicators.rsi[i] > indicators.rsi[prevIdx]) ? 1 : 0,
      // RSI keluar dari oversold pada selected date (cross di atas 30)
      rsiExitOversold: (prevIdx >= 0 && indicators.rsi[i] && indicators.rsi[prevIdx] &&
        indicators.rsi[prevIdx] < 30 && indicators.rsi[i] >= 30) ? 1 : 0,
      // RSI dalam zona bullish awal (30-50) pada selected date - potensi naik
      rsiBullishZone: (indicators.rsi[i] && indicators.rsi[i] >= 30 && indicators.rsi[i] <= 50) ? 1 : 0,
      
      // RSI 7 Momentum: RSI7 naik pada selected date
      rsi7Rising: (prevIdx >= 0 && indicators.rsi7[i] && indicators.rsi7[prevIdx] &&
        indicators.rsi7[i] > indicators.rsi7[prevIdx]) ? 1 : 0,
      rsi7ExitOversold: (prevIdx >= 0 && indicators.rsi7[i] && indicators.rsi7[prevIdx] &&
        indicators.rsi7[prevIdx] < 30 && indicators.rsi7[i] >= 30) ? 1 : 0,
      rsi7BullishZone: (indicators.rsi7[i] && indicators.rsi7[i] >= 30 && indicators.rsi7[i] <= 50) ? 1 : 0,
      
      // RSI 21 Momentum
      rsi21Rising: (prevIdx >= 0 && indicators.rsi21[i] && indicators.rsi21[prevIdx] &&
        indicators.rsi21[i] > indicators.rsi21[prevIdx]) ? 1 : 0,
      rsi21ExitOversold: (prevIdx >= 0 && indicators.rsi21[i] && indicators.rsi21[prevIdx] &&
        indicators.rsi21[prevIdx] < 30 && indicators.rsi21[i] >= 30) ? 1 : 0,
      rsi21BullishZone: (indicators.rsi21[i] && indicators.rsi21[i] >= 30 && indicators.rsi21[i] <= 50) ? 1 : 0,
      
      // Stochastic Golden Cross (%K cross di atas %D) pada selected date
      stochGoldenCross: (prevIdx >= 0 && indicators.stoch[i] && indicators.stoch[prevIdx] &&
        indicators.stoch[prevIdx].k <= indicators.stoch[prevIdx].d &&
        indicators.stoch[i].k > indicators.stoch[i].d) ? 1 : 0,
      // Stochastic keluar dari oversold pada selected date
      stochExitOversold: (prevIdx >= 0 && indicators.stoch[i] && indicators.stoch[prevIdx] &&
        indicators.stoch[prevIdx].k < 20 && indicators.stoch[i].k >= 20) ? 1 : 0,
      
      // Volume surge dengan harga naik pada selected date (bullish volume)
      bullishVolume: (indicators.volumeRatio[i] && indicators.volumeRatio[i] > 1.2 && 
        resampledPrices[i].close > resampledPrices[i].open) ? 1 : 0,
      // Volume spike pada selected date (>2x average)
      volumeSpike: indicators.volumeRatio[i] && indicators.volumeRatio[i] > 2 ? 1 : 0,
      
      // Price near lower Bollinger Band pada selected date (potential bounce zone)
      nearLowerBB: (indicators.bb[i] && currentClose <= indicators.bb[i].lower * 1.02) ? 1 : 0,
      // Price bouncing dari lower BB pada selected date
      bouncingFromLowerBB: (prevIdx >= 0 && indicators.bb[i] && indicators.bb[prevIdx] &&
        resampledPrices[prevIdx].close <= indicators.bb[prevIdx].lower &&
        currentClose > indicators.bb[i].lower) ? 1 : 0,
      // BB Width sempit pada selected date (potensi breakout)
      bbSqueeze: (indicators.bb[i] && 
        ((indicators.bb[i].upper - indicators.bb[i].lower) / indicators.bb[i].middle * 100) < 5) ? 1 : 0,
      
      // ADX Rising pada selected date (trend strengthening)
      adxRising: (prevIdx >= 0 && indicators.adx[i] && indicators.adx[prevIdx] &&
        indicators.adx[i].adx > indicators.adx[prevIdx].adx) ? 1 : 0,
      // Bullish DI Cross (+DI cross di atas -DI) pada selected date
      bullishDICross: (prevIdx >= 0 && indicators.adx[i] && indicators.adx[prevIdx] &&
        indicators.adx[prevIdx].pdi <= indicators.adx[prevIdx].mdi &&
        indicators.adx[i].pdi > indicators.adx[i].mdi) ? 1 : 0,
      
      // Hammer/Bullish Reversal Candle Pattern pada selected date
      hammerCandle: (currentHigh - currentLow > 0 &&
        (Math.min(currentOpen, currentClose) - currentLow) > 
        (currentHigh - currentLow) * 0.6 &&
        Math.abs(currentClose - currentOpen) < 
        (currentHigh - currentLow) * 0.3) ? 1 : 0,
      // Bullish engulfing pada selected date
      bullishEngulfing: (prevIdx >= 0 && 
        resampledPrices[prevIdx].close < resampledPrices[prevIdx].open && // Previous bearish
        currentClose > currentOpen && // Current bullish
        currentClose > resampledPrices[prevIdx].open && // Body engulfs
        currentOpen < resampledPrices[prevIdx].close) ? 1 : 0,
      
      // Multi-indicator bullish confluence score untuk selected date (0-10)
      bullishScore: (() => {
        let score = 0;
        // RSI bullish signals pada selected date
        if (indicators.rsi[i] < 40) score += 1; // Not overbought
        if (indicators.rsi[i] > indicators.rsi[prevIdx]) score += 1; // Rising RSI
        // MACD bullish signals pada selected date
        if (indicators.macd[i]?.histogram > indicators.macd[prevIdx]?.histogram) score += 1; // Histogram improving
        if (indicators.macd[i]?.MACD > indicators.macd[i]?.signal) score += 1; // Bullish MACD
        // Volume confirmation pada selected date
        if (indicators.volumeRatio[i] > 1.2) score += 1; // Above average volume
        if (indicators.obv[i] > indicators.obv[prevIdx]) score += 1; // OBV rising
        // Trend indicators pada selected date
        if (indicators.adx[i]?.pdi > indicators.adx[i]?.mdi) score += 1; // Bullish DI
        if (currentClose > currentOpen) score += 1; // Bullish candle
        // Price position pada selected date
        if (indicators.stoch[i]?.k < 50) score += 1; // Not overbought stoch
        if (currentClose > indicators.sma20[i]) score += 1; // Above SMA20
        return score;
      })(),
      
      // Calculate 52-week (250-day) high and low from selected date
      highestHigh52w: (() => {
        const period = Math.min(250, i + 1);
        if (period < 2) return null;
        const prices52w = resampledPrices.slice(Math.max(0, i - period + 1), i + 1);
      ...
      distFromHigh52w: (() => {
        ...
        return ((high52w - currentClose) / high52w) * 100; // % distance from 52w high
      })(),
      
      // 1 jika harga jauh dari 52-week high (>30%) on selected date
      farFromHigh52w: (() => {
        const period = Math.min(250, i + 1);
        ...
        const dist = ((high52w - currentClose) / high52w) * 100;
        return dist > 30 ? 1 : 0;
      })(),
      
      // 1 jika harga sangat jauh dari 52-week high (>50%) on selected date
      veryFarFromHigh52w: (() => {
        const period = Math.min(250, i + 1);
        if (period < 2) return 0;
        const prices52w = resampledPrices.slice(Math.max(0, i - period + 1), i + 1);
      ...
      // % distance from 52-week low (positive = above low) on selected date
      distFromLow52w: (() => {
        const period = Math.min(250, i + 1);
        if (period < 2) return null;
        const prices52w = resampledPrices.slice(Math.max(0, i - period + 1), i + 1);
        ...
        return ((currentClose - low52w) / low52w) * 100; // % distance dari 52w low
      })(),
      
      // 1 jika dekat dengan 52-week low (<10%) on selected date
      nearLow52w: (() => {
        const period = Math.min(250, i + 1);
        if (period < 2) return 0;
        const prices52w = resampledPrices.slice(Math.max(0, i - period + 1), i + 1);
        ...
        const dist = ((currentClose - low52w) / low52w) * 100;
        return dist < 10 ? 1 : 0;
      })(),
      
      // Calculate support level (lowest low dari 50 hari terakhir) up to selected date
      supportLevel50d: (() => {
        const period = Math.min(50, i + 1);
        if (period < 2) return null;
        const prices50 = resampledPrices.slice(Math.max(0, i - period + 1), i + 1);
        return Math.min(...prices50.map(p => p.low));
      })(),
      
      // Calculate resistance level (highest high dari 50 hari terakhir) up to selected date
      resistanceLevel50d: (() => {
        const period = Math.min(50, i + 1);
        if (period < 2) return null;
        const prices50 = resampledPrices.slice(Math.max(0, i - period + 1), i + 1);
      ...
      // % distance dari support level on selected date
      distFromSupport: (() => {
        const period = Math.min(50, i + 1);
        if (period < 2) return null;
        const prices50 = resampledPrices.slice(Math.max(0, i - period + 1), i + 1);
        ...
        return ((currentClose - supportLvl) / supportLvl) * 100;
      })(),
      
      // 1 jika dekat dengan support level (<5%) on selected date
      nearSupport: (() => {
        const period = Math.min(50, i + 1);
        if (period < 2) return 0;
        const prices50 = resampledPrices.slice(Math.max(0, i - period + 1), i + 1);
        ...
        const dist = ((currentClose - supportLvl) / supportLvl) * 100;
        return dist < 5 ? 1 : 0;
      })(),
      
      // 1 jika dekat dengan support tapi tidak terlalu jauh dari high (recovery potential) on selected date
      recoveryPotential: (() => {
        const period52w = Math.min(250, i + 1);
        const period50d = Math.min(50, i + 1);
        if (period52w < 2 || period50d < 2) return 0;
        
        const prices52w = resampledPrices.slice(Math.max(0, i - period52w + 1), i + 1);
        const prices50 = resampledPrices.slice(Math.max(0, i - period50d + 1), i + 1);
        ...
        const distFromSup = ((currentClose - supportLvl) / supportLvl) * 100;
        const distFromHigh = ((high52w - currentClose) / high52w) * 100;
        
        return (distFromSup < 8 && distFromHigh > 20) ? 1 : 0;
      })(),

      // Parabolic SAR Reversal Detection (on selected date)
      psar: safeToFixed(indicators.psar[i]),
      psarAbovePrice: indicators.psar[i] && currentClose < indicators.psar[i] ? 1 : 0,
      psarBelowPrice: indicators.psar[i] && currentClose > indicators.psar[i] ? 1 : 0,
      psarNearPrice: (() => {
        if (!indicators.psar[i]) return 0;
        const distance = Math.abs(currentClose - indicators.psar[i]);
        const percentDist = (distance / currentClose) * 100;
        return percentDist < 2 ? 1 : 0; // Jika SAR dalam 2% dari harga
      })(),
      psarBearishReversal: (() => {
        if (prevIdx < 0 || !indicators.psar[i] || !indicators.psar[prevIdx]) return 0;
        // Reversal dari bullish ke bearish: SAR cross dari bawah ke atas
        const prevWasBullish = resampledPrices[prevIdx].close > indicators.psar[prevIdx];
        const nowBearish = currentClose < indicators.psar[i];
        return (prevWasBullish && nowBearish) ? 1 : 0;
      })(),
      psarAboutToReversal: (() => {
        if (!indicators.psar[i] || i < 5) return 0;
      ...
      psarBullishReversal: (() => {
        ...
        const prevWasBearish = resampledPrices[prevIdx].close < indicators.psar[prevIdx];
        const nowBullish = currentClose > indicators.psar[i];
        return (prevWasBearish && nowBullish) ? 1 : 0;
      })(),
      psarAboutToBullishReversal: (() => {
        if (!indicators.psar[i] || i < 5) return 0;
        // SAR di atas harga dan sangat dekat (<3%), akan pindah ke bawah
        if (indicators.psar[i] <= currentClose) return 0; // SAR sudah di bawah, skip
        const distance = indicators.psar[i] - currentClose;
        const percentDist = (distance / currentClose) * 100;
        return percentDist < 3 ? 1 : 0; // Warning: SAR di atas tapi dekat, akan bullish reversal
      })(),
      
      // Composite signals (on selected date)
      oversoldBounce: ((indicators.rsi[i] < 35 || indicators.stoch[i]?.k < 25) &&
        currentClose > currentOpen && // Bullish candle
        indicators.volumeRatio[i] > 1) ? 1 : 0,
      
      momentumShift: (indicators.macd[i]?.histogram > indicators.macd[prevIdx]?.histogram &&
        indicators.rsi[i] > indicators.rsi[prevIdx] &&
        indicators.adx[i]?.pdi > indicators.adx[i]?.mdi) ? 1 : 0,
    };
  ...
  getIntradayIndicators(prices) {
    ...
    const row = {
      ...
      deltaRSI7: safeToFixed(deltaRSI7),
      deltaRSI21: safeToFixed(deltaRSI21),
      ...
      // RSI 7
      rsi7: safeToFixed(indicators.rsi7[i]),
      rsi7Oversold: indicators.rsi7[i] && indicators.rsi7[i] < 30 ? 1 : 0,
      rsi7Overbought: indicators.rsi7[i] && indicators.rsi7[i] > 70 ? 1 : 0,
      rsi7Neutral: indicators.rsi7[i] && indicators.rsi7[i] >= 30 && indicators.rsi7[i] <= 70 ? 1 : 0,
      
      // RSI 21
      rsi21: safeToFixed(indicators.rsi21[i]),
      rsi21Oversold: indicators.rsi21[i] && indicators.rsi21[i] < 30 ? 1 : 0,
      rsi21Overbought: indicators.rsi21[i] && indicators.rsi21[i] > 70 ? 1 : 0,
      rsi21Neutral: indicators.rsi21[i] && indicators.rsi21[i] >= 30 && indicators.rsi21[i] <= 70 ? 1 : 0,
