...
class IndicatorService {
  ...
  generateRegressionDataset(prices, startDate, endDate, options = {}) {
    ...
    for (let i = 2; i < prices.length - horizon; i++) { // Need future data for H+n
      ...
      // === TP/SL SIMULATION ===
      // Entry = close hari sinyal (H, = currentClose). TP/SL dicek harian dari H+1 s/d H+n (hari perdagangan).
      ...
      let tpslExitIdx = futureIndex;
      let tpslExitPrice = futureClose;
      let tpslDaysHeld = horizon;
      for (let d = 1; d <= horizon; d++) {
        const day = prices[i + d];
        if (!day) break;
      ...
      const row = {
        ...
        tpslStatus,
        tpslExitDate: prices[tpslExitIdx]?.date || prices[futureIndex].date,
